...
def upgrade() -> None:
    ...
    op.alter_column(
        "channels", "phone_number_id", new_column_name="meta_phone_number_id"
    )
    op.alter_column("channels", "business_id", new_column_name="meta_business_id")
    op.add_column(
        "channels", sa.Column("meta_waba_id", sa.String(length=255), nullable=True)
    )
    op.add_column(
        "channels",
        sa.Column(
            ...
            nullable=False,
            server_default="WhatsApp Channel",
        ),
    )
    ...
    op.create_index(
        "idx_channel_meta_id", "channels", ["meta_phone_number_id"], unique=True
    )
    ...
    op.create_table(
        "contact_channel_states",
        sa.Column("id", sa.Uuid(), nullable=False),
        sa.Column("workspace_id", sa.Uuid(), nullable=False),
        sa.Column("contact_id", sa.Uuid(), nullable=False),
        sa.Column("channel_id", sa.Uuid(), nullable=False),
        sa.Column(
            "opt_in_status", sa.Boolean(), nullable=False, server_default="false"
        ),
    ...
    op.create_index(
        "idx_ccs_channel_optin",
        "contact_channel_states",
        ["channel_id", "opt_in_status"],
        unique=False,
    )
    op.create_index(
    ...
    op.create_index(
        "idx_ccs_workspace", "contact_channel_states", ["workspace_id"], unique=False
    )
    op.create_index(
        "idx_ccs_workspace_contact",
        "contact_channel_states",
    ...
    op.create_foreign_key(
        None, "campaigns", "channels", ["channel_id"], ["id"], ondelete="CASCADE"
    )
    ...
    # Campaign Messages
    op.alter_column(
        "campaign_messages", "phone_number_id", new_column_name="channel_id"
    )
    op.execute(
        "ALTER TABLE campaign_messages DROP CONSTRAINT IF EXISTS campaign_messages_phone_number_id_fkey"
    )
    op.create_foreign_key(
        None,
        "campaign_messages",
    ...
    op.create_foreign_key(
        None, "conversations", "channels", ["channel_id"], ["id"], ondelete="CASCADE"
    )
    ...
    op.execute(
        "ALTER TABLE messages DROP CONSTRAINT IF EXISTS messages_phone_number_id_fkey"
    )
    op.create_foreign_key(
        None, "messages", "channels", ["channel_id"], ["id"], ondelete="CASCADE"
    )
    op.execute("DROP INDEX IF EXISTS idx_msg_phone_time")
    op.create_index(
        "idx_msg_channel_time", "messages", ["channel_id", "created_at"], unique=False
    )

    # Templates
    op.alter_column("templates", "phone_number_id", new_column_name="channel_id")
    op.execute(
        "ALTER TABLE templates DROP CONSTRAINT IF EXISTS templates_phone_number_id_fkey"
    )
    op.create_foreign_key(
        None, "templates", "channels", ["channel_id"], ["id"], ondelete="CASCADE"
    )
    ...
    op.execute(
        "ALTER TABLE webhook_logs DROP CONSTRAINT IF EXISTS webhook_logs_phone_number_id_fkey"
    )
    op.create_foreign_key(
        None, "webhook_logs", "channels", ["channel_id"], ["id"], ondelete="SET NULL"
    )
    op.execute("DROP INDEX IF EXISTS idx_webhook_phone_time")
    op.create_index(
        "idx_webhook_channel_time",
        "webhook_logs",
        ["channel_id", "received_at"],
        unique=False,
    )
    ...
    op.create_index(
        "idx_contact_source_channel", "contacts", ["source_channel_id"], unique=False
    )
    op.create_foreign_key(
        None, "contacts", "channels", ["source_channel_id"], ["id"], ondelete="SET NULL"
    )
...
def downgrade() -> None:
    ...
    op.alter_column(
        "phone_numbers", "meta_phone_number_id", new_column_name="phone_number_id"
    )
