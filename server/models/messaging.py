...
class Conversation(TimestampMixin, Base):
    ...
    phone_number_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("phone_numbers.id", ondelete="CASCADE"), nullable=False)
    
    status: Mapped[str] = mapped_column(String(20), default="open", nullable=False)
    conversation_type: Mapped[str] = mapped_column(String(30), default="user_initiated", nullable=False)
    ...
    assigned_to: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("workspace_members.id", ondelete="SET NULL"), nullable=True)
    ...
    assignee: Mapped[Optional["WorkspaceMember"]] = relationship("WorkspaceMember", back_populates="assigned_conversations")
...
class MediaFile(TimestampMixin, SoftDeleteMixin, Base):
    ...
    uploaded_by: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("workspace_members.id", ondelete="SET NULL"), nullable=True)
...
class Message(Base):
    ...
    media_id: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("media_files.id", ondelete="SET NULL"), nullable=True)
    ...
    sent_by: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("workspace_members.id", ondelete="SET NULL"), nullable=True)
    ...
    sender: Mapped[Optional["WorkspaceMember"]] = relationship("WorkspaceMember", back_populates="sent_messages")
    campaign_message: Mapped[Optional["CampaignMessage"]] = relationship("CampaignMessage", back_populates="message")
