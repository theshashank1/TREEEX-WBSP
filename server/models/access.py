...
import uuid  # <--- CRITICAL FIX: Must import the python module
...
class User(TimestampMixin, SoftDeleteMixin, Base):
    ...
    # CRITICAL FIX: mapped_column(Uuid...) not mapped_column(uuid...)
...
class Workspace(TimestampMixin, SoftDeleteMixin, Base):
    ...
    # Cross-file relationships (using string references)
    phone_numbers: Mapped[List["PhoneNumber"]] = relationship("PhoneNumber", back_populates="workspace", cascade="all, delete-orphan")
...
class WorkspaceMember(TimestampMixin, Base):
    ...
    assigned_conversations: Mapped[List["Conversation"]] = relationship("Conversation", back_populates="assignee")
